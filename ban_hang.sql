...
--test
select maSP, sum(so_luong) from don_hang where the_loai=1  group by(maSP) order by(sum(so_luong)) desc limit 1;
SELECT * FROM laptop order by(ma) desc LIMIT 3 OFFSET 6;
select * from laptop order by(ma) desc limit 3;

delete from don_hang where trang_thai='Chờ xác nhận';
update camera set noi_lap_dat='Ngoài trời' where ma=28;
select * from don_hang;
ALTER TABLE nguoi_dung DROP COLUMN xac_thuc;

update laptop set so_luong=so_luong-12 where ma='7';
ALTER TABLE laptop ADD giam_gia varchar(255);
ALTER TABLE camera
ADD noi_Lap_dat varchar(255);
-- drop table quan_tri

ALTER TABLE don_hang MODIFY gia bigint;
